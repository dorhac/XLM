...
#
N_MONO=1200000  # number of monolingual sentences for each language
...
if ! [[ -f "$TGT_RAW" ]]; then
 ...
 cat $(ls $TGT/*$TGT* | grep -v gz) | head -n $N_MONO > $TGT_RAW
fi
...
echo "hip $(wc -l < $TGT_RAW) rock $(wc -l < $SRC_RAW)"
...
# echo "Applying BPE to valid and test files..."
 ...
 #$FASTBPE applybpe $PARA_SRC_VALID_BPE $MySRC_PATH $BPE_CODES $SRC_VOCAB
 #$FASTBPE applybpe $PARA_SRC_TEST_BPE $MySRC_PATH $BPE_CODES $SRC_VOCAB
 #$FASTBPE applybpe $PARA_TGT_VALID_BPE $MyTGT_PATH $BPE_CODES $TGT_VOCAB
 #$FASTBPE applybpe $PARA_TGT_TEST_BPE  $MyTGT_PATH $BPE_CODES $TGT_VOCAB
